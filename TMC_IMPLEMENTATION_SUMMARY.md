...
- Config TLV tags: 0x0040-0x0049 for READK index, SN, auth code, offsets/locations
...
**File**: `applet-tmc/src/main/java/org/openjavacard/ndef/tmc/NdefApplet.java` (1021 lines)
...
5. Generate COUNTER(3) + AUTH_CODE(5) + RANDOM(8) → AES-CBC encrypt → hex-encode at ENC_LOCATION
6. Compute CBC-MAC over (URL data + raw SN + raw dynamic ciphertext) with ISO 9797 method 2 padding → hex-encode 8 bytes at MAC_LOCATION
7. Increment 3-byte read counter
8. Return requested portion
